...
/// @title AscICO contract - Takes funds from users and issues tokens.
...
contract AscICO {
     ...
     * External contracts
     */
    AscToken public ascToken = AscToken(0x0);
        ...
        returns (uint)
    {
        ...
        if (!ascToken.issueTokens(msg.sender, tokenCount)) {
        ...
        returns (uint)
    {
        ...
        if (!ascToken.issueTokens(beneficiary, _tokenCount)) {
            // Tokens could not be issued.
            throw;
        }
        return _tokenCount;
        ...
        returns (bool)
    {
        if (isICOActive == true) {
            ...
            if (!ascToken.issueTokens(multisig, founderBonus)) {
                // Tokens could not be issued.
                throw;
            }
        }
    ...
    function AscICO(address _multisig) {
        // Set founder address
        founder = msg.sender;
